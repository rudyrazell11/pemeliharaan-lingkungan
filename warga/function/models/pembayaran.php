...
{
    global $koneksi;
    $items = $koneksi->query("SELECT * FROM pembayaran pmb INNER JOIN warga ON pmb.id_warga=warga.id_warga INNER JOIN periode_iuran pi ON pi.id_periode_iuran=pmb.id_periode_iuran INNER JOIN jenis_iuran ji ON pi.id_jenis_iuran=ji.id_jenis_iuran WHERE warga.id_user = $_SESSION[id_user]  ORDER BY pmb.id_pembayaran DESC ");
    $data = [];
